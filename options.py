...
class Option(NetOption):
	def __init__(self, conf_path):
		...
		self.dataset = self.conf['dataset']  # options: carvana | nih
		self.dir_img = self.conf['dir_img']  # the path root of image data
		self.dir_mask = self.conf['dir_mask']  # the path root of mask data
		self.model_path = self.conf['model_path']  # the path root of pre-trained model
		self.img_scale = self.conf['img_scale']
	...
	def paramscheck(self, logger):
		...
		if self.dataset in ["carvana", "nih"]:
			self.nClasses = 2